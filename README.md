# Balancing-Robot-RaspberryPi [(Current Status)](https://www.youtube.com/watch?v=j3caxAyNHA4&index=1&list=PLveRMPt4kAsA41ivMscrzFSWWx54CI52J)
...
- **Position Maintain Capability**: The robot tends to drift due to the fact that the chassis is not balanced. So I implemented position maintaining capabities to it. I used a **"P** only controller here. This controller gives an input to the outer Cascade loop an input velocity proportional to how many encoder counts away the robot is from the reference position. This velocity is directed towards the initial starting position. To prevent overshoot, I implemented braking logic before the robot reaches the reference position so as to minimise overshoot. 
